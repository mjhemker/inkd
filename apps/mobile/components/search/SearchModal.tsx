/**
 * Global search (mobile) — a MODAL that expands over the current view, not a
 * pushed navigation screen. A dimmed backdrop (tap to close) sits behind an
 * opaque search surface anchored to the top: a back arrow + search field at the
 * top, results below. There is exactly ONE loading indicator, in the results
 * area (the old pushed screen showed two — one in the bar, one in the body).
 *
 ...
 * affordance via the SearchModal provider.
 */
...
export function SearchModal({ open, onClose }: SearchModalProps) {
  ...
  const { results, isLoading, isEmpty, query: debounced, count } = useGlobalSearch(query);

  // Load recents whenever the modal opens; clear the field on each open.
  useEffect(() => {
    if (!open) return;
    setQuery("");
  ...
  const go = useCallback(
    ...
    [debounced, recents, persistRecents, onClose],
  );
  ...
  // Cap the surface so a dimmed, tappable backdrop always shows beneath it.
  const maxSurface = Dimensions.get("window").height * 0.82;
  ...
  return (
    ...
    >
      {/* Dimmed backdrop — tap anywhere outside the surface to close. */}
      <Pressable className="flex-1 bg-black/60" onPress={onClose}>
        <Pressable
          // Stop taps on the surface from closing.
          onPress={(e) => e.stopPropagation()}
          style={{ maxHeight: maxSurface }}
          className="overflow-hidden rounded-b-3xl bg-surface-base"
        >
          <SafeAreaView edges={["top"]}>
            ...
            {/* Results area — the single loading indicator lives here. */}
            {debounced.length < 2 ? (
              <RecentsList
                recents={recents}
                onPick={setQuery}
