/**
 * Studio → Settings body — tabbed editor mirroring the web settings-view.tsx.
 * Reuses the same shared editors as onboarding.tsx (components/artist/*), each
 * rendered in `variant="settings"` so they show their own Save button. Header +
 * segmented bar are owned by StudioScreen; this renders only the settings body.
 */
...
/** Old `?tab=` keys (and new keys) → the grouped tab they now live on. */
            ...
            >
              <View className="gap-4 rounded-xl border border-border-subtle p-5">
                <Text className="text-sm text-content-secondary">
                  Manage your MD/PA consent forms, edit template content, and review signed
                  waivers from clients.
                </Text>
                <Button onPress={() => router.push("/waivers")} className="self-start">
                  Manage waivers
                  <Icon name="arrow-right" size={16} color={colors.text.primary} />
                </Button>
              </View>
        ...
        {tab === "preferences" && (
          <View className="gap-10">
            ...
            >
              <View className="gap-4 rounded-xl border border-border-subtle p-5">
                <Text className="text-sm text-content-secondary">
                  Dark is the gallery default; Light is a warm paper wall. System follows your
                  device.
                </Text>
                <AppearanceControl />
...
/** Placard section header + its editor, used inside merged settings tabs. */
...
/** Artist toggle for the aftercare healing timeline. */
...
function SwitchToClientCard() {
  ...
  return (
    <View className="gap-3 rounded-xl border border-border-subtle p-5">
      ...
      <Text className="text-sm text-content-secondary">
        Step back from being an artist and use INKD to get tattooed. Your studio stays intact —
        nothing is deleted.
      </Text>
      ...
      >
        <View className="gap-2">
          ...
          <Text className="text-sm text-content-secondary">
            • Your bookings, portfolio and signed waivers are kept but frozen — never deleted.
          </Text>
          <Text className="text-sm text-content-secondary">
            • Your navigation switches to the client experience.
          </Text>
          <Text className="text-sm text-content-muted">
            Becoming an artist again is invite/setup-based during the pilot — reach out and
...
function DangerZoneCard() {
  ...
  return (
    <View className="gap-3 rounded-xl border border-danger-500/40 bg-danger-500/5 p-5">
      ...
      <Text className="text-sm text-content-secondary">
        Permanently delete your INKD account and all of your data. This cannot be undone.
      </Text>
      <Button
        variant="outline"
      ...
      >
        <View className="gap-2">
          <Text className="text-sm text-content-secondary">Type DELETE to confirm.</Text>
          <Input
            value={confirmText}
