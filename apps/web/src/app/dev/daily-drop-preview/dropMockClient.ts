/**
 * In-memory Supabase stand-in for the offline DAILY DROP preview harness.
 * Mirrors dev/feed-preview/feedMockClient.ts (this sandbox blocks egress to the
 * Supabase host, so a mock is the only way to render/screenshot the surface),
 * with two additions the drop surface exercises: the `daily_drops` table and the
 * `.is(col, null)` filter used by the seen/clicked/react stamps.
 *
...
class MockBuilder implements PromiseLike<{ data: unknown; error: null }> {
  private filters: Array<{ kind: "eq" | "in" | "is"; col: string; val: unknown }> = [];
  ...
  private matches(row: Row): boolean {
    return this.filters.every((f) => {
      if (f.kind === "in") return Array.isArray(f.val) && f.val.includes(row[f.col]);
      if (f.kind === "is") return (row[f.col] ?? null) === (f.val ?? null);
...
export interface DropMockSeed {
  viewerId: string | null;
  email: string;
  tables: Record<string, Row[]>;
}

export function createDropMockClient(seed: DropMockSeed): InkdSupabaseClient {
  const db = new MockDb();
